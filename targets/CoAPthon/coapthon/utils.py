...
def with_coverage(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            cov.stop()
            # Directly collect coverage data without saving to disk
            coverage_data = {}
            for filename in cov.get_data().measured_files():
                lines = cov.get_data().lines(filename)
                coverage_data[filename] = lines

            # Function to attach coverage data to the response object
            def attach_coverage_data(obj):
                if hasattr(obj, 'payload'):
                    original_payload = obj.payload
